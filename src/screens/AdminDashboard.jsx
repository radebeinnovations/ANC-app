...
export default function AdminDashboard() {
  ...
  async function load() {
    if (!isSupabaseConfigured()) { setAccessError('Supabase is not configured for this deployment.'); setLoading(false); return; }
    ...
    } catch (nextError) { setAccessError(nextError?.message || 'Dashboard access could not be confirmed.'); }
  ...
  if (!scope || accessError) return <View style={[s.authPage, isMobile && { padding: 16 }]}><View style={[s.authCard, isMobile && { padding: 22 }]}><Text style={s.brand}>ANC UNITY</Text><Text style={s.authTitle}>Dashboard access required</Text><Text style={s.authCopy}>{accessError || 'This account does not have a dashboard role.'}</Text><TouchableOpacity style={s.primaryButton} onPress={load}><Text style={s.primaryButtonText}>Try again</Text></TouchableOpacity><TouchableOpacity onPress={signOut}><Text style={s.signOut}>Sign out</Text></TouchableOpacity></View></View>;
  ...
  return <ScrollView contentContainerStyle={[s.page, isMobile && { padding: 16, gap: 14 }]} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.scope}><Icon name="verified-user" size={18} color={Colors.primary}/><Text style={s.scopeText}>{canPublishNational ? 'Super Admin · create National or Branch events' : `Branch Organiser · events for ${scope.branch_name || 'your assigned branch'} only`}</Text></View>
    {error ? <Text style={s.error}>{error}</Text> : null}
    <View style={[s.stats, isMobile && { flexDirection: 'column', gap: 10 }]}>{summary.map(item => <View key={item.label} style={[s.stat, isMobile && { flex: 0, width: '100%' }]}><Text style={s.statNumber}>{item.value}</Text><Text style={s.statLabel}>{item.label}</Text></View>)}</View>
    <View style={[s.grid, isMobile && { flexDirection: 'column', gap: 14 }]}>
      <View style={[s.panel, s.gridPanel, isMobile && { width: '100%' }]}>
        <Text style={s.panelTitle}>Publish an event</Text><Text style={s.panelCopy}>{canPublishNational ? 'Choose whether the event is visible nationally or only to a named branch.' : `Events are published only to ${scope.branch_name}.`}</Text>
        <TextInput value={form.title} onChangeText={value => setForm(current => ({ ...current, title: value }))} placeholder="Event title" style={s.input}/><Text style={s.fieldLabel}>Choose start date</Text><EventCalendar value={eventDate} time={eventTime} onChangeDate={setEventDate} onChangeTime={setEventTime}/>
        <TextInput value={form.venue} onChangeText={value => setForm(current => ({ ...current, venue: value }))} placeholder="Venue" style={s.input}/><TextInput value={form.location} onChangeText={value => setForm(current => ({ ...current, location: value }))} placeholder="Location / city" style={s.input}/>
        {canPublishNational && form.audience === 'branch' ? <TextInput value={form.branch} onChangeText={value => setForm(current => ({ ...current, branch: value }))} placeholder="Branch name for this event" style={s.input}/> : null}
        <Text style={s.fieldLabel}>Event audience</Text><View style={s.audienceRow}><TouchableOpacity accessibilityRole="button" onPress={() => setForm(current => ({ ...current, audience: 'branch' }))} style={[s.audience, form.audience === 'branch' && s.audienceActive]}><Text style={[s.audienceText, form.audience === 'branch' && s.audienceTextActive]}>Branch</Text></TouchableOpacity>{canPublishNational ? <TouchableOpacity accessibilityRole="button" onPress={() => setForm(current => ({ ...current, audience: 'national', branch: '' }))} style={[s.audience, form.audience === 'national' && s.audienceActive]}><Text style={[s.audienceText, form.audience === 'national' && s.audienceTextActive]}>National</Text></TouchableOpacity> : null}</View>
        <TextInput value={form.description} onChangeText={value => setForm(current => ({ ...current, description: value }))} placeholder="Description for members" multiline style={[s.input, s.description]}/><TouchableOpacity disabled={saving} style={[s.primaryButton, saving && s.disabled]} onPress={publishEvent}><Text style={s.primaryButtonText}>{saving ? 'Publishing…' : 'Publish event'}</Text></TouchableOpacity>
      </View>
      <View style={[s.panel, s.gridPanel, isMobile && { width: '100%' }]}><Text style={s.panelTitle}>Published events</Text>{(dashboard?.events || []).length ? dashboard.events.map(event => <View key={event.id} style={s.event}><Text style={s.eventTitle}>{event.title}</Text><Text style={s.eventMeta}>{new Date(event.starts_at).toLocaleString()} · {event.audience === 'national' ? 'National' : event.branch_name}</Text><Text style={s.eventMeta}>{event.venue || event.location || 'Venue to be confirmed'} · {event.rsvp_count || 0} RSVPs</Text></View>) : <Text style={s.empty}>No events have been published in your scope yet.</Text>}</View>
    </View>
