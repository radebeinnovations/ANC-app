...
export default function SignInScreen({ finish, onSignIn, onAuthSuccess, onBackToWelcome }) {
  ...
  async function submit() {
    ...
    if (mode === 'signup' && (!fullName.trim() || !membershipNumber.trim() || !branchName.trim())) { setMessage('Complete your full name, membership number, and branch.'); return; }
    setLoading(true);
    try {
      if (mode === 'signup') {
        const { data, error } = await supabase.auth.signUp({ email: email.trim().toLowerCase(), password, options: { data: { full_name: fullName.trim(), membership_number: membershipNumber.trim().toUpperCase(), branch_name: branchName.trim() } } });
