...
export default function ChatScreen({ user, onRequireSignIn }) {
  ...
  async function load() {
    ...
    try {
      ...
      // Do not request permission automatically. If the member has already
      // granted it, however, prioritise verified phone-contact matches before
      // the broader ANC member suggestions every time Chat is opened.
      let contacts = { matches: [] };
      try {
        contacts = await readContactMatches();
      } catch {
        // Contact matching is an enhancement; the rest of Chat must still
        // load if a phone's address book is temporarily unavailable.
      }
      setConversations(chatList);
      setPeople(prioritizeContactMatches(suggested, contacts.matches));
    } catch (error) {
  ...
  async function findPhoneContacts() {
    ...
    setContactStatus('');
    try {
      const contacts = await readContactMatches({ requestPermission: true });
      if (contacts.webOnly) {
      ...
      if (!contacts.hasPhoneNumbers) {
        setContactStatus('No phone numbers were found in your contacts.');
        return;
      }
      setPeople(current => prioritizeContactMatches(current, contacts.matches));
      setContactStatus(contacts.matches.length ? `${contacts.matches.length} ANC ${contacts.matches.length === 1 ? 'member is' : 'members are'} in your contacts. They now appear first below.` : 'No ANC members from your contacts have joined yet.');
    } catch (error) {
...
async function readContactMatches({ requestPermission = false } = {}) {
  if (Platform.OS === 'web') return { webOnly: true, granted: false, hasPhoneNumbers: false, matches: [] };
  const permission = requestPermission ? await Contacts.requestPermissionsAsync() : await Contacts.getPermissionsAsync();
  if (permission.status !== 'granted') return { webOnly: false, granted: false, hasPhoneNumbers: false, matches: [] };

  const response = await Contacts.getContactsAsync({ fields: [Contacts.Fields.PhoneNumbers] });
  const phoneNumbers = [...new Set((response.data || [])
    .flatMap(contact => (contact.phoneNumbers || []).map(phone => normalizePhone(phone.number)))
    .filter(Boolean))];
  if (!phoneNumbers.length) return { webOnly: false, granted: true, hasPhoneNumbers: false, matches: [] };

  const matches = await getMembersInContacts(phoneNumbers.slice(0, 1000));
  return { webOnly: false, granted: true, hasPhoneNumbers: true, matches };
}

function prioritizeContactMatches(suggested = [], contactMatches = []) {
  const seen = new Set();
  return [...contactMatches.map(person => ({ ...person, matched_contact: true })), ...suggested]
    .filter(person => {
