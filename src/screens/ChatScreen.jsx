...
export default function ChatScreen({ user, onRequireSignIn }) {
  ...
  useEffect(() => {
    ...
    unsubscribe = subscribeToMessages(selected.id, message => setMessages(current => current.some(existing => existing.id === message.id) ? current : [...current, message]));
    typingChannel = createTypingChannel(selected.id, members => setTyping(members.some(member => member.typing && member.name !== currentUser.full_name)));
  ...
  if (selected) return <KeyboardAvoidingView style={s.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={s.chatHeader}><TouchableOpacity onPress={() => setSelected(null)} style={s.back}><Icon name="arrow-back" size={22} /><View style={s.avatar}><Text style={s.avatarText}>{selected.initials}</Text></View><View><Text style={s.chatName}>{selected.name}</Text><Text style={s.presence}>{typing ? 'typing…' : selected.type === 'group' ? `${selected.member_count || 0} members` : 'Online'}</Text></View></TouchableOpacity></View><FlatList ref={listRef} data={messages} keyExtractor={item => item.id} contentContainerStyle={s.messages} onContentSizeChange={() => listRef.current?.scrollToEnd({ animated: true })} renderItem={({ item }) => <View style={[s.bubble, item.sender_id === currentUser.id && s.bubbleMine]}><Text style={[s.bubbleText, item.sender_id === currentUser.id && s.bubbleTextMine]}>{item.content}</Text><Text style={[s.messageTime, item.sender_id === currentUser.id && s.messageTimeMine]}>{new Date(item.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text></View>} /><View style={s.composer}><TextInput value={composer} onChangeText={updateComposer} onKeyPress={handleComposerKeyPress} returnKeyType="send" placeholder="Message" placeholderTextColor={Colors.muted} style={s.composerInput} multiline /><TouchableOpacity accessibilityRole="button" accessibilityLabel="Send message" disabled={!composer.trim()} onPress={submit} style={[s.sendButton, !composer.trim() && s.sendDisabled]}><Icon name="send" color={Colors.white} size={20}/></TouchableOpacity></View></KeyboardAvoidingView>;
